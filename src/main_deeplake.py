...
def retriever_eval():
    ...
    accuracy_df = pd.DataFrame(columns=["Chunk Size", "Doc Match Count", "Chunk Match Count"])
    ...
    for chunk_size in chunk_sizes:
        ...
        accuracy_df = pd.concat([accuracy_df, pd.DataFrame([[chunk_size, doc_match_count, chunk_match_count]], columns=accuracy_df.columns)])

        # save dataframe csv
        csv_path = os.path.join(src_dir, f'query_results_{chunk_size}.csv')
        df_results.to_csv(csv_path, index=False)
        print(f"The DataFrame has been saved to '{csv_path}'.")
        print()

    # Save the accumulated results to a CSV
    accuracy_csv_path = os.path.join(src_dir, 'query_results_chunkComparison.csv')
