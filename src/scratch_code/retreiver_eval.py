...
def parse_search_results_csv_old(file_path: str) -> Dict[str, Dict]:
    id_to_metadata = {}
    with open(file_path, 'r', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
...
def parse_ead_xml_old(file_path: str) -> Dict[str, Dict]:
    ead_metadata = {}
    try:
        tree = ET.parse(file_path)
        root = tree.getroot()
...
def parse_marc_xml_old(file_path: str) -> Dict[str, Dict]:
    marc_metadata = {}
    try:
        # Use pymarc to parse the XML file
        records = parse_xml_to_array(file_path)
...
def process_metadata_old(data_dir: str) -> Dict[str, Dict]:
    # file paths
...
def load_data_old(data_dir, metadata):
    documents = []
    txt_dir = os.path.join(data_dir, 'txt')
...
def generate_embeddings_old(dataset_path, chunked_documents, embeddor):
...
def search_vector_store_old(query, vectorstore, top_k, filter=None):
    try:
...
def retriever_eval():
    ...
    vstore_dir = os.path.join(project_root, 'data', 'vectorestore')
    ...
    for model_name in model_names:
        for top_k in top_ks:
            for chunk_size in chunk_sizes:
                ...
                # Load data and chunk
                logger, documents = load_data(data_dir, metadata)
                ...
                #logger, vectorstore = generate_embeddings(dataset_path, logger, chunked_documents, embeddor)
                vectorstore = DeepLake.from_dataset(vstore_dir)
                ...
                # reset
                del documents, logger, chunked_documents, metadata, scores, possible_filenames, embeddor, vectorstore
                gc.collect()
    ...
    # Save the detailed results and the comparable df to a CSV
    df_results_path = os.path.join(eval_dir, 'test_on_all.csv')
    compare_df_path = os.path.join(eval_dir, 'test_on_all_compare.csv')
    df_results.to_csv(df_results_path, index=False)
