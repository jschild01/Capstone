...
def retriever_eval():
    ...
    accuracy_df = pd.DataFrame(columns=["Chunk Size", "Embedding model", "Doc Match Count", "Chunk Match Count"])

    model_names = ['instructor', 'mini'] # titan in the future when functioning
    chunk_sizes = [100, 250, 500, 800, 1000, 1500, 2000]
    ...
    for chunk_size in chunk_sizes:
        for model_name in model_names:
            ...
            accuracy_df = pd.concat([accuracy_df, pd.DataFrame([[chunk_size, model_name, doc_match_count, chunk_match_count]], columns=accuracy_df.columns)])

            # save dataframe csv
            csv_path = os.path.join(eval_dir, f'query_results_{model_name}{chunk_size}.csv')
